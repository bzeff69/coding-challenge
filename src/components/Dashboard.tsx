...
export function Dashboard({ habits, deletedHabits, onClose }: DashboardProps) {
  ...
  const handleExportPdf = useCallback(() => {
    ...
    printWindow.document.write(`<!DOCTYPE html><html><head><title>Streak Dashboard</title><style>
      * { margin: 0; padding: 0; box-sizing: border-box; }
      body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; padding: 32px; background: ${isDark ? '#0c0a09' : '#fff'}; color: ${isDark ? '#fafaf9' : '#1c1917'}; }
      .stat-card { border-radius: 12px; padding: 12px; color: white; break-inside: avoid; }
      .stat-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px; }
      .habit-row { display: flex; align-items: center; gap: 12px; padding: 12px; border-radius: 8px; background: ${isDark ? '#1c1917' : '#fafaf9'}; margin-bottom: 8px; break-inside: avoid; }
      .progress-bar { height: 6px; border-radius: 999px; background: ${isDark ? '#44403c' : '#e7e5e4'}; overflow: hidden; flex: 1; }
      .progress-fill { height: 100%; border-radius: 999px; }
      .heatmap-cell { width: 18px; height: 18px; border-radius: 3px; display: inline-block; margin: 1.5px; }
      .section-title { font-size: 14px; font-weight: 600; margin-bottom: 8px; color: ${isDark ? '#d6d3d1' : '#44403c'}; }
      .muted { color: ${isDark ? '#78716c' : '#a8a29e'}; }
      h1 { font-size: 24px; font-weight: 800; margin-bottom: 4px; }
      .subtitle { font-size: 13px; color: ${isDark ? '#78716c' : '#a8a29e'}; margin-bottom: 24px; }
      .footer { text-align: center; margin-top: 32px; font-size: 11px; color: ${isDark ? '#57534e' : '#d6d3d1'}; }
      @media print { body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } }
    </style></head><body>
      <h1>Streak Dashboard</h1>
      <div class="subtitle">Exported ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
      ${content}
      <div class="footer">Generated by Streak — Track habits. Get judged.</div>
    </body></html>`);
